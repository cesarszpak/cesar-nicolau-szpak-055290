...
const list = (page = 0, size = 9) => api.get(`/api/albuns?page=${page}&size=${size}`) as Promise<Page<Album>>

const listByArtist = (artistaId: number, page = 0, size = 9) => api.get(`/api/albuns/artista/${artistaId}?page=${page}&size=${size}`) as Promise<Page<Album>>

const get = (id: number) => api.get(`/api/albuns/${id}`) as Promise<Album>

const searchByName = (nome: string, page = 0, size = 9) => api.get(`/api/albuns/buscar/nome?nome=${encodeURIComponent(nome)}&page=${page}&size=${size}`) as Promise<Page<Album>>
